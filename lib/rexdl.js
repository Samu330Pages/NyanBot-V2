...
async function rexdl(query, pageCount) {
    ...
    try {
        for (let i = 1; i <= pageCount; i++) {
            ...
            const urls = await page.$$eval('article', articles =>
                articles.map(article => {
                    const titleElement = article.querySelector('h2.post-title a');
                    const iconElement = article.querySelector('.post-thumbnail img');
                    return {
                        url: titleElement?.href,
                        title: titleElement?.innerText,
                        iconURL: iconElement?.getAttribute('data-src') || iconElement?.src
                    };
                })
            );

            for (const { url, title, iconURL } of urls) {
