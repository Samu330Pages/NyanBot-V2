...
class SearchRexdl {
    ...
    async rexdl(pageCount) {
        ...
        try {
            for (let i = 1; i <= pageCount; i++) {
                await page.setUserAgent(await this.randomUserAgent());
                await page.goto(`https://rexdl.com/page/${i}/?s=${this.query}`, { waitUntil: 'domcontentloaded' });
    ...
    async getDownloadDetails(appUrl) {
        const browser = await puppeteer.launch({ headless: true });
        const page = await browser.newPage();
