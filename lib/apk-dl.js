...
let apkcombo = {
  search: async function(args) {
    ...
    $('div.content-apps > a').each(function(a, b) {
      ...
      let link = $(b).attr('href').replace('https://apkcombo-com.translate.goog/', 'https://apkcombo.com/').replace('/?_x_tr_sl=en&_x_tr_tl=fr&_x_tr_hl=en&_x_tr_pto=wapp', ''); // URL de la aplicación
      let author = $(b).find('.author').text();
      let description = $(b).find('.description').text();
      let imgSrc = $(b).find('img').attr('data-src');
      let downloads = description.match(/(\d+\sM\+|\d+\sK\+)/)?.[0];
      let rating = description.match(/(\d+,\d+\s★|\d+\s★)/)?.[0];
      let size = description.match(/(\d+\sGB|\d+\sMB)/)?.[0];
