...
// brat Image
const brat = async ({ text = 'Nyan ™' }) => {
    try {
        const words = decodeURIComponent(text).split(' ');
        const canvas = createCanvas(118, 118);
        const ctx = canvas.getContext('2d');
        ...
        words.forEach(word => {
            ...
            let totalHeight = (lines.length + 1) * fontSize * 1.2;
            while (textWidth > 98 || totalHeight > 118) {
                fontSize -= 1;
                ctx.font = `${fontSize}px Arial`;
                textWidth = ctx.measureText(word).width;
                totalHeight = (lines.length + 1) * fontSize * 1.2;
            }
            lines.push(word);
        });
