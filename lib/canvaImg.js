...
const createWelcomeImage = async (profilePicUrl) => {
    ...
    const darkBackgroundColor = 'rgba(0, 0, 0, 0.5)'; // Color oscuro para el fondo del texto
    ctx.fillStyle = darkBackgroundColor;
    ctx.fillRect(100, 80, canvas.width - 200, 150); // Fondo oscuro para el texto
    ...
    ctx.fillText('¡Bienvenido al grupo!', canvas.width / 2, 150); // Ajustar la posición del texto

    ctx.strokeStyle = '#FFFFFF';
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.moveTo(50, 590);
    ctx.lineTo(canvas.width - 50, 590);
    ctx.stroke();

    ctx.fillText('NyanBotV2 te da la bienvenida', canvas.width / 2, 630);
