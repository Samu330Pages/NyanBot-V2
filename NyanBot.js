...
if (m.quoted && m.quoted.text && /(?:https?:\/\/)?(?:www\.)?(?:youtube\.com\/(?:[^\/\n\s]+\/\S+\/|(?:v|e(?:mbed)?)\/|.*[?&]v=|shorts\/)|youtu\.be\/)([a-zA-Z0-9_-]{11})/.test(m.quoted.text) && m.quoted.id.startsWith("NYAN330")) {
...
case 'rw':
    ...
    if (db.data.users[sender].rewards.includes(code)) {
        return reply(`Ya has reclamado esta recompensa. 😒`);
    }
    
    db.data.users[sender].limit += puntos;
    db.data.users[sender].rewards.push(code);
